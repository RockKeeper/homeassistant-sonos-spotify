...
from homeassistant.helpers.entity_registry import RegistryEntry, EntityRegistry
...
from homeassistant.components.spotify import media_player as homeassistant_spotify
...
from .const import (
    DATA_SPOTIFY_SONOS,
    DOMAIN as SPOTIFY_SONOS_DOMAIN,
    SONOS_ALBUM,
    SONOS_ALBUM_ARTIST,
    SONOS_ARTIST,
    SONOS_COMPOSER,
...
ATTR_SPOTIFY_SONOS_GROUP = "spotify_sonos_group"
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    if DATA_SPOTIFY_SONOS not in hass.data:
        hass.data[DATA_SPOTIFY_SONOS] = SpotifySonosData()

    config = hass.data[SPOTIFY_SONOS_DOMAIN].get("media_player", {})
    ...
    def _stop_discovery(event):
        data = hass.data[DATA_SPOTIFY_SONOS]
    ...
    def _discovery(now=None):
        ...
        def _discovered_player(soco):
            ...
            try:
                ...
                if soco.uid not in hass.data[DATA_SPOTIFY_SONOS].discovered:
                    _LOGGER.debug("Adding new entity")
                    hass.data[DATA_SPOTIFY_SONOS].discovered.append(soco.uid)
                    hass.add_job(async_add_entities, [SpotifySonosEntity(soco)])
                else:
        ...
        if hosts:
            ...
            hass.data[DATA_SPOTIFY_SONOS].hosts_heartbeat = hass.helpers.event.call_later(
                DISCOVERY_INTERVAL, _discovery
            )
        else:
            _LOGGER.debug("Starting discovery thread")
            hass.data[DATA_SPOTIFY_SONOS].discovery_thread = pysonos.discover_thread(
                _discovered_player,
    ...
    @service.verify_domain_control(hass, SPOTIFY_SONOS_DOMAIN)
    async def async_service_handle(service_call: ServiceCall):
        ...
        if service_call.service == SERVICE_JOIN:
            ...
            if master:
                await SpotifySonosEntity.join_multi(hass, master, entities)
            else:
                _LOGGER.error(
            ...
            await SpotifySonosEntity.unjoin_multi(hass, entities)
        elif service_call.service == SERVICE_SNAPSHOT:
            await SpotifySonosEntity.snapshot_multi(
                hass, entities, service_call.data[ATTR_WITH_GROUP]
            )
        elif service_call.service == SERVICE_RESTORE:
            await SpotifySonosEntity.restore_multi(
                hass, entities, service_call.data[ATTR_WITH_GROUP]
            )

    hass.services.async_register(
        SPOTIFY_SONOS_DOMAIN,
        SERVICE_JOIN,
    ...
    hass.services.async_register(
        SPOTIFY_SONOS_DOMAIN, SERVICE_SNAPSHOT, async_service_handle, join_unjoin_schema
    )

    hass.services.async_register(
        SPOTIFY_SONOS_DOMAIN, SERVICE_RESTORE, async_service_handle, join_unjoin_schema
    )
...
class _ProcessSpotifySonosEventQueue:
...
def _get_entity_from_soco_uid(hass, uid):
    """Return SpotifySonosEntity from SoCo uid."""
    _LOGGER.debug("Reached _get_entity_from_soco_uid, uid=%s", uid)

    for entity in hass.data[DATA_SPOTIFY_SONOS].entities:
...
class SpotifySonosEntity(MediaPlayerEntity):
    ...
    async def async_added_to_hass(self):
        ...
        self.hass.data[DATA_SPOTIFY_SONOS].entities.append(self)

        def _rebuild_groups():
            """Build the current group topology."""
            for entity in self.hass.data[DATA_SPOTIFY_SONOS].entities:
    ...
    def device_info(self):
        ...
        return {
            "identifiers": {(SPOTIFY_SONOS_DOMAIN, self._unique_id)},
            "name": self._name,
            "model": self._model.replace("SpotifySonos ", ""),
    ...
    def _attach_player(self):
        ...
        try:
            ...
            def subscribe(sonos_service, action):
                ...
                queue = _ProcessSpotifySonosEventQueue(action)
    ...
    def update_groups(self, event=None):
        ...
        async def _async_handle_group_event(event):
            ...
            async with self.hass.data[DATA_SPOTIFY_SONOS].topology_condition:
                group = await _async_extract_group(event)

                if self.unique_id == group[0]:
                    _async_regroup(group)

                    self.hass.data[DATA_SPOTIFY_SONOS].topology_condition.notify_all()
    ...
    def play_media(self, media_type, media_id, **kwargs):
        ...
        """
        media_id = "x-sonos-spotify:" + media_id
    ...
    async def join_multi(hass, master, entities):
        """Form a group with other players."""
        async with hass.data[DATA_SPOTIFY_SONOS].topology_condition:
            group = await hass.async_add_executor_job(master.join, entities)
            await SpotifySonosEntity.wait_for_groups(hass, [group])
    ...
    async def unjoin_multi(hass, entities):
        ...
        async with hass.data[DATA_SPOTIFY_SONOS].topology_condition:
            ...
            await SpotifySonosEntity.wait_for_groups(hass, [[e] for e in entities])
    ...
    async def restore_multi(hass, entities, with_group):
        ...
        async with hass.data[DATA_SPOTIFY_SONOS].topology_condition:
            ...
            await SpotifySonosEntity.wait_for_groups(hass, groups)
        ...
        try:
            with async_timeout.timeout(5):
                while not _test_groups(groups):
                    await hass.data[DATA_SPOTIFY_SONOS].topology_condition.wait()
    ...
    def device_state_attributes(self):
        ...
        attributes = {ATTR_SPOTIFY_SONOS_GROUP: [e.entity_id for e in self._sonos_group]}
    ...
    async def async_browse_media(self, media_content_type=None, media_content_id=None):
        ...
        entity_id = "media_player.<spotify media player name>"
        entity_reg = EntityRegistry(self.hass)
        await entity_reg.async_load()
        entity = entity_reg.async_get(entity_id)
        spotifyAccount = self.hass.data["spotify"]["<unique entity id>"]["spotify_me"]
        spotifySession = self.hass.data["spotify"]["<unique entity id>"]["spotify_session"]
        spotifyClient = self.hass.data["spotify"]["<unique entity id>"]["spotify_client"]

        self._scope_ok = set(spotifySession.token["scope"].split(" ")).issuperset(
            SPOTIFY_SCOPES
        )
        ...
        if media_content_type in [None, "library"]:
            return await self.hass.async_add_executor_job(homeassistant_spotify.library_payload)
        ...
        response = await self.hass.async_add_executor_job(
            build_item_response, spotifyClient, spotifyAccount, payload
        )
        if response is None:
            raise BrowseError(
        ...
        return entity.async_browse_media(media_content_type, media_content_id)
...
def build_item_response(spotify, user, payload):
    ...
    if "images" in media:
        params["thumbnail"] = homeassistant_spotify.fetch_image_url(media)
    elif image:
...
def item_payload(item):
    ...
    return homeassistant_spotify.item_payload(item)
...
def library_payload(media_library):
    ...
    Used by async_browse_media.
    """
    return homeassistant_spotify.library_payload()
